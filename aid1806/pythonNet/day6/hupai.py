...
class ChuanMa(object):
    ...
    def __init__(self,pai):
        ...
        self.num = int(pai[0])
        self.view = pai
        self.id = ChuanMa.pattern.index(self.pt)*10 + self.num 
        #编码，通过间隔一个数子，达到区分花色的目的


class InputParse(object):
    @classmethod
    def split_pai(cls,paimian):
        result =  []
...
class PaiAnaly(object):
    ...
    def find_ok_three(sort_list):
        ...
        for j in range(idx1+1,len(sort_list)): 
        #找到sort_list[0]+1 的情况下，寻找sort_list[0]+2
        ...
        if idx2 == 0:
            return False
        return sort_list[idx1],sort_list[idx2]  #若找到，则返回


    @staticmethod
    def recur_check(lt):  #递归过程
        if len(lt) != 0:   #当列表为空是，就可以认为胡牌
            re = PaiAnaly.find_ok_three(lt)
            if not re:
                return False
            else:   #如果依然能够配对，就祛除已经配对的牌，继续递归调用
                lt.remove(lt[0])
                lt.remove(re[0])
                lt.remove(re[1])
                print(lt)
    ...
    def find_duizi(self): #找出手牌中的所有的对子，
    #然后以每个对子作为头，调用以上的递归过程
        res = []
        for i in range(13):
            try:
                #print (self.id_list[i])
                if self.id_list[i] == self.id_list[i+1]\
                and self.id_list[i] != self.id_list[res[-1]]:
                    res.append(i)
            except IndexError:
                res.append(i)
        print(res)
...
if __name__ == '__main__':
    ...
    pai_instance_list = []
    for p in pai_list:
        pai_instance_list.append(ChuanMa(p))  
